...
import {
  ...
  const MOCK_USDC_ADDR = process.env.NEXT_PUBLIC_USDC_ADDRESS_SEPOLIA as `0x${string}`;
  ...
  const gen6 = () => String(Math.floor(Math.random() * 1_000_000)).padStart(6, "0");
   ...
   * - If empty, generates a code, ensures uniqueness via `inviteCodeOwner(code) == address(0)`,
  ...
  export async function getOrCreateInviteCode(
    ...
    for (let i = 0; i < 10; i++) {
      ...
      // Uniqueness check via the mapping getter
    ...
    throw new Error("Could not find a free 6-digit code after several attempts. Try again.");
  }
  ...
  export async function approveUsdcSpending(
    ...
    if (!PRESALE_ADDR) throw new Error("PRESALE address is missing");
    if (!MOCK_USDC_ADDR) throw new Error("USDC address is missing");
    if (!(humanAmountUsdc > 0)) throw new Error("Amount must be greater than 0");
  
    // Read token decimals (your mock uses 6, but this keeps it robust)
    const decimals: number = await readContract({ contract: mockUsdc, method: "decimals" });
 ...
 * - round: 0 = Private, 1 = Institutional, 2 = Community (per your enum)
...
export async function buyPresaleTokens(
    ...
    if (!PRESALE_ADDR) throw new Error("PRESALE address is missing");
    if (!MOCK_USDC_ADDR) throw new Error("USDC address is missing");
    if (!(humanUsdcAmount > 0)) throw new Error("USDC amount must be greater than 0");
    if (!inviteCode || inviteCode.trim().length === 0) throw new Error("Invite code is required");
  
    // Read USDC decimals and convert the human amount to base units
